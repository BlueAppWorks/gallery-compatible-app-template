...
1. Gallery Operator からアプリのインフラ（Compute Pool, Service, Postgres Instance）を **発見** できる
2. Gallery Operator に OPERATE/MONITOR 権限を GRANT することで **Compute Pool の起動/停止を委任** できる
3. `app_setup.resume_service()` プロシージャを提供し、Gallery Operator が SERVICE を **明示的に起動** できる
4. Gallery Operator が SERVICE のエンドポイントを **検出** してユーザーに URL を提供できる
...
### 5.4 必須: Gallery Operator 検知パターン

Native App のサンドボックス制約により `SHOW APPLICATIONS` で他アプリの存在を検知できない。
Gallery Operator は consumer setup で固定名データベース `BLUE_APP_GALLERY_REGISTRY` を作成する。
各アプリはこのテーブルを参照して Operator の存在を判定する。

> **重要**: `ROLE PUBLIC` への GRANT だけでは Native App サンドボックスから
> 外部データベースにアクセスできない。**APPLICATION ごとに明示的な GRANT が必要。**

#### 権限付与（ACCOUNTADMIN で実行）
...
**setup.sql の実行時点では検知不可。** インストール時はまだレジストリへの GRANT がないため。
検知は **Streamlit UI 起動後** に行う。

#### Streamlit での検知（Python）— リファレンス実装
...
```python
if not gallery_operator_installed:
    app_name = session.sql("SELECT CURRENT_DATABASE()").collect()[0][0]
    st.warning(
        "**Gallery Operator を検出できません。**\n\n"
        "以下のいずれかの状態です:\n"
        "- Gallery Operator が未インストール\n"
        "- インストール済みだが、このアプリへのアクセス権限が未付与\n\n"
        "Gallery Operator インストール済みの場合、**ACCOUNTADMIN** で以下を実行してください:"
    )
    st.code(
        f"-- ACCOUNTADMIN で実行\n"
...
> インストール時のアプリ名が返る。GRANT SQL をアプリ名に応じて動的生成できる。
...
### Gallery Operator 検知（Streamlit UI で実装）

- [ ] Streamlit UI に Gallery Integration セクションを追加
- [ ] `BLUE_APP_GALLERY_REGISTRY.PUBLIC.OPERATOR` を参照して Operator の存在を判定
- [ ] Operator 未検出時: レジストリへの GRANT SQL を表示（`CURRENT_DATABASE()` で動的生成）
- [ ] Operator 検出時: gallery_managed トグル + GRANT APPLICATION ROLE SQL を表示
- [ ] **注意**: setup.sql での auto-GRANT は不可（インストール時はレジストリにアクセスできない）
...
- [ ] `app_setup.resume_service()` プロシージャを作成
- [ ] `resume_service()` を APPLICATION ROLE に GRANT
...
- [ ] Streamlit App Management でアプリを発見 → `is_managed = TRUE`
- [ ] `app_catalog.gallery_compatible = TRUE` に設定
- [ ] `validate_managed_app` で全リソースが GRANTED であることを確認
- [ ] SaaS Gallery から `api.launch()` → エンドポイント到達を確認
...
SaaS はリクエストを Operator に送信する前にティア制限を適用する。
これは SaaS 側のビジネスロジックであり、Operator は関知しない。

| ティア | 最大リース時間 | 同時リース数 |
|---|---|---|
| free | 60 分 | 1 |
| pro | 480 分 | 5 |
| enterprise | 1440 分 | 50 |

### 10.3 Launch フロー（SaaS 視点）

```
1. ユーザーが Gallery UI で「Launch」をクリック
2. SaaS がティア制限をチェック（duration, concurrent leases）
3. CALL BLUE_APP_GALLERY.api.launch(app_name, duration, user)
